<?php require_once("assets/config/config.php"); ?>
<!DOCTYPE html>
<!--[if lt IE 7]>   <html class="no-js lt-ie9 lt-ie8 lt-ie7" lang="es"> <![endif]-->
<!--[if IE 7]> 		<html class="no-js lt-ie9 lt-ie8" lang="es"> <![endif]-->
<!--[if IE 8]> 		<html class="ie8 no-js" lang="es"> <![endif]-->
<!--[if IE 9]> 		<html class="ie9 no-js" lang="es"> <![endif]-->
<!--[if !IE]><!-->
<html lang="es">
<!--<![endif]-->

<head>
     <meta charset="utf-8">
     <meta name="author" content="Alejandro Pérez-Moneo Nieto">
     <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
     <meta name="description" content="">
     <title>Analytics — Landing Page</title>
     <link rel="stylesheet" href="assets/css/dist/output.css" type="text/css" />

</head>

<body class="container mx-auto max-w-screen-2xl bg-darkblue">
     <?php include("assets/templates/header.php"); ?>
     <main class="bg-white bg-header ">
          <section class="">
               <div>
                    <h1>Monitor your business on real-time dashboard</h1>
                    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Elementum nisi aliquet volutpat pellentesque volutpat est. Sapien in etiam vitae nibh nunc mattis imperdiet sed nullam. </p>
                    <button class="">Try for free</button>
               </div>
               <img src="assets\images\main-screenDoble.svg" alt="">
          </section>
          <section class="container mx-auto bg-white">
               <h2>Main Features</h2>
               <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Elementum nisi aliquet volutpat pellentesque volutpat est. Sapien in etiam vitae nibh nunc mattis imperdiet sed nullam. Vitae et, tortor pulvinar risus pulvinar sit amet. Id vel in nam malesuada.</p>
               <div class="">
                    <div>
                         <img src="assets\images\planning.svg" alt="">
                         <h3>Monitoring 24/7</h3>
                         <p>Lorem ipsum dolor sit amet, consectetur adipis cing elit. Elementum nisi aliquet volutpat.</p>
                    </div>
                    <div>
                         <img src="assets\images\computer.svg" alt="">
                         <h3>Widget System</h3>
                         <p>Sapien in etiam vitae nibh nunc mattis imperdiet sed nullam. Vitae et, tortor pulvinar risus pulvinar.</p>
                    </div>
                    <div>
                         <img src="assets\images\speed.svg" alt="">
                         <h3>Best Performance</h3>
                         <p>Lorem ipsum dolor sit amet, consectetur adipis cing elit. Elementum nisi aliquet volutpat.</p>
                    </div>
               </div>
          </section>
          <section class="">
               <div class="">
                    <h2>Automated Reports & Widget Alerts</h2>
                    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Elementum nisi aliquet volutpat pellentesque volutpat est. Sapien in etiam vitae nibh nunc mattis imperdiet sed nullam. Vitae et, tortor pulvinar risus pulvinar sit amet.</p>
               </div>
               <img src="assets\images\screen-01.svg" alt="" srcset="">
          </section>
          <section class="">
               <img src="assets\images\screen-02.svg" alt="" srcset="">
               <div class="">
                    <h2>Fully customizable to address your needs</h2>
                    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Elementum nisi aliquet volutpat pellentesque volutpat est. Sapien in etiam vitae nibh nunc mattis imperdiet sed nullam. Vitae et, tortor pulvinar risus pulvinar sit amet.</p>
               </div>
          </section>
          <section class="">
               <div class="">
                    <h2>Pre-built Dashboard Templates</h2>
                    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Elementum nisi aliquet volutpat pellentesque volutpat est. Sapien in etiam vitae nibh nunc mattis imperdiet sed nullam. Vitae et, tortor pulvinar risus pulvinar sit amet.</p>
               </div>
               <img src="assets\images\screen-03.svg" alt="" srcset="">
          </section>
          <section class="bg-darkblue">
               <h2 class="text-4xl font-bold text-center text-white">Pricing Plans</h2>
               <p class="text-center text-secondary mt-7">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Elementum nisi aliquet volutpat pellentesque volutpat est. </p>
               <div class="grid grid-cols-3 gap-20 m-auto mt-16 text-center place-items-center">
                    <div class="rounded-[30px] w-80 bg-azuloscuro hover:bg-white">
                         <h3 class="pt-12 pb-2 text-3xl font-bold text-white hover:text-main">Starter</h3>
                         <span class="text-lg text-secondary ">up to 3 users</span>
                         <div class="w-56 h-[1px] bg-secondary mx-auto"></div>
                         <div class="mt-16 text-white hover:text-main">
                              $29
                         </div>
                         <div><span class="text-base text-secondary">per month</span></div>
                         <button class="px-20 py-2 mt-10 mb-16 text-base text-white border hover:text-main rounded-4xl">Order</button>
                    </div>
                    <div class="card">
                         <h3 class="font-bold text-white hover:text-main">Standard</h3>
                         <span>up to 20 users</span>
                         <div class="text-white hover:text-main">
                              $99<span>per month</span>
                         </div>
                         <button class="text-white hover:text-main">Order</button>
                    </div>
                    <div class="rounded[30px] w-80 bg-azuloscuro hover:bg-white">
                         <h3 class="font-bold text-white hover:text-main">Premium</h3>
                         <span>up to 200 users</span>
                         <div class="text-white hover:text-main"
                              $299<span>per month</span>
                         </div>
                         <button class="text-white hover:text-main">Order</button>
                    </div>
               </div>
          </section>
     </main>
     <?php include("assets/templates/footer.php"); ?>
     <script src="assets/js/custom.js"></script>
</body>
</html>